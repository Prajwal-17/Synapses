...
export default function Connection({ connectionId, appType, stepNo }: { connectionId: string, appType: string, stepNo: number }) {
  ...
  const fetchConnections = async () => {
    ...
    try {
      const response = await fetch(`/api/connections?app=${appType}&userId=${session?.user.id}`, {
        method: "GET"
      })
  ...
  return (
    ...
    <div className="w-full flex justify-between items-center">
      ...
      >
        ...
        <SelectContent >
          {connections ? (
            connections.map((item: ConnectionType, index) => (
              <SelectItem key={index} value={item.metaData.email}>{item.metaData.email}</SelectItem>
            ))
          ) : (
            <div>
              Loading...
